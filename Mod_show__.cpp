f// ПРОЕКТ:     Математическая модель ИМПУЛЬС
// ВЕРСИЯ:     1.0
// ФАЙЛ:       Mod_Show.CPP
// ДАТА:       8/96
// АВТОР:      Штифанов А.И.
...
//!!!!!!!!!!!!!!!!!  SelectObject(hDC, State.hFont);
  ...
  if (ews == SHOW_CHANNEL) {
	 ...
	 Ntrub = Parameters.Ntrub;
	 float Delitel = Parameters.Delitel;
...
//  MessageBox(Application->Handle, str, "Отладка", MB_ICONASTERISK|MB_OK);
  ...
  int deltaColor = 255  / Parameters.QEquLine;
  int green = 0;
...
*/
	 ...
	 long int iElectr = Parameters.xCoord - rModel.left;
	 long int jElectr = rModel.bottom - Parameters.yCoord;
...
//     sprintf (str, "[Fn...Fn+1] = [%.4f...%.4f]", ElLine*deltaF, (ElLine+1)*deltaF);
  ...
  y +=20;
  sprintf (str, "dF = %.4lf (диапазон приращения функции)", deltaF);
  TextOut(hDC, x, y, str, lstrlen(str));
...
void Legenda(HDC hDC, int x, int y, char *str, COLORREF Color)
{
 ...
 TextOut(hDC, i+15, y-5, str, lstrlen(str));
  ...
  DWORD j, jN1, i;
  COLORREF color5, color4, color2, color6, color8;
  for (j=0; j<=M; j++)  {
     jN1 = j*N1;
     if (GetAsyncKeyState(VK_ESCAPE)) return;
     for (i=0; i<=N; i++)  {
     	Knots[jN1+i].color = BGIColor[WHITE];
        double Value;
        switch(ews)  {
           case SHOW_EQULINES_PSI: Value = Knots[jN1+i].Psi;
           break;
        ...
        if(Value != 0.0 && Value  < MaxValue )  {
           int ni = (int)ceil(Value  / deltaF);
           if (ni % 2) Knots[jN1+i].color = BGIColor[BLACK];
           else        Knots[jN1+i].color = BGIColor[WHITE];
        }
     }
  }
  for (j=0; j<=M; j++)  {
     ...
     for (i=0; i<=N; i++)  {
         if (Knots[jN1+i].ePhisic==IT_POINT_CENTER)  {
            color5 = Knots[jN1+i].color;
            color6 = Knots[jN1+i+1].color;
            color4 = Knots[jN1+i-1].color;
            color2 = Knots[(j-1)*N1+i].color;
            color8 = Knots[(j+1)*N1+i].color;
            if (color6==color5 && color4==color5 &&
                color2==color5 && color8==color5) color5 = BGIColor[WHITE];
            else if (color6==color5 && color4==color5 &&
                     color2==color5 && color8==color5) color5 = BGIColor[WHITE];
                 else color5 = BGIColor[BLACK];
         }
         else color5 = BGIColor[BLACK];

         x = rModelN.left + i*Interval;
         y = rModelN.bottom - j*Interval;
         DrawKnot(hDC, x, y, r, color5, ScaleShow);
     }
  }
  ---------------------------------------------------*/
